...
class ResumeParser:
    ...
    def clean_company_name(self, company):
        ...
        noise_terms = ['education', 'university', 'college', 'institute', '\n', '•', 'relevant coursework']
        company = ' '.join(word for word in company.split() if not any(term.lower() in word.lower() for term in noise_terms))
        ...
        try:
            ...
            for ent in doc.ents:
                if ent.label_ == "ORG":
                    ...
                    if any(edu_term in company.lower() for edu_term in ['university', 'college', 'institute', 'school']):
                        continue
                    
                    context_window = text[max(0, ent.start_char-150):min(len(text), ent.end_char+150)]
                    ...
                    if titles and dates:
                        experience_entries.append({
                            ...
                            'date_range': dates[0],
                            'description': self._extract_description(context_window)
                        })
                        seen_companies.add(company)
            
            return [entry for entry in experience_entries if entry['company'] and entry['title']]
        except Exception as e:
    ...
    def _extract_description(self, text, max_length=200):
        ...
        try:
            ...
            if descriptions:
                ...
                description = ' '.join(desc.strip() for desc in descriptions[:2])  # Take first 2 points
        ...
        try:
            ...
            for ent in doc.ents:
                if ent.label_ == "ORG":
                    ...
                    if not any(keyword in institution.lower() for keyword in edu_keywords):
                        continue
                        
                    if institution in seen_institutions:
                        continue
                    
                    context_window = text[max(0, ent.start_char-200):min(len(text), ent.end_char+200)]
                    ...
                    # Extract dates
                    date_matches = re.findall(r"20\d{2}\s*(?:-|–|to)\s*(?:20\d{2}|present)", context_window)

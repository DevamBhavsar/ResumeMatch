            ...
            if text.strip().startswith('<'):
                ...
                for element in soup.find_all(['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'ul', 'ol', 'div']):
                    if element.name.startswith('h'):
                        # Handle headings
                        level = int(element.name[1])
                        heading_style = styles[f'Heading{level}'] if f'Heading{level}' in styles else styles['Heading1']
                        
                        # Clean the HTML for ReportLab
                        clean_html = DocumentGenerator._clean_html_for_reportlab(str(element))
                        ...
                        for li in element.find_all('li', recursive=False):
                            ...
                            clean_html = DocumentGenerator._clean_html_for_reportlab(str(li))
                            
                            list_items.append(ListItem(Paragraph(clean_html, normal_style)))
                        
                        list_style = 'bulletlist' if element.name == 'ul' else 'orderedlist'
